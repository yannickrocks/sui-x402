...
export type SimulateResult =
  | {
      $kind: "Transaction";
      Transaction: {
        effects: { gasUsed: GasCostSummary };
 ...
 * supplied through `chainIdentifiers`. A mainnet value may only ever land here
 * through a human's verification against two independent sources, recorded with
 * both sources and the date — the process is docs/facilitator-runbook.md
 * "Mainnet" and this repo's mainnet-enablement issues.
 */
...
export const PROTOCOL_MAX_TX_GAS = 50_000_000_000_000n;
/** Same fixed overhead the Sui SDK adds to a simulated computation cost. */
const GAS_SAFE_OVERHEAD = 1000n;

export type PaymentBuildReason =
  | "network_mismatch"
  | "simulation_failed"
  | "gas_budget_exceeded"
  | "self_check_failed"
  /** SUI-denominated payment offered for sponsorship. */
  | "unsupported_sponsored_asset"
  /** Sponsor returned bytes whose kind differs from the submitted kind. */
  | "sponsor_altered_payment"
  /** Sponsor response failed a sender/gasOwner/digest/budget check. */
  | "sponsor_response_invalid";

export class PaymentBuildError extends Error {
  constructor(readonly reason: PaymentBuildReason, message: string) {
    super(message);
...
export function computeGasBudget(
  ...
  if (!Number.isInteger(headroomPercent) || headroomPercent < 0) {
    throw new RangeError(
      `gasHeadroomPercent must be a non-negative integer, got ${headroomPercent}`
    );
  }
  const base = BigInt(gasUsed.computationCost) + GAS_SAFE_OVERHEAD * gasPrice;
  const withStorage =
    base + BigInt(gasUsed.storageCost) - BigInt(gasUsed.storageRebate);
...
export function receivedBy(
  ...
  for (const bc of changes) {
    if (
      normalizeSuiAddress(bc.address) !== payTo ||
      normalizeStructTag(bc.coinType) !== asset
    )
      continue;
    found = true;
...
/**
 * The wrong-chain guard shared by the self-funded and sponsored builders: the
 * offer's network must resolve to a pinned genesis digest, and the client must
 * actually be talking to that chain.
 */
export function assertChainIdentifier(
  ...
  const expectedChain = { ...CHAIN_IDENTIFIERS, ...overrides }[
...
async function simulate(
  ...
  if (result.$kind === "FailedTransaction") {
    ...
    throw new PaymentBuildError(
      "simulation_failed",
      `payment simulation failed: ${detail}`
    );
  }
...
export async function buildPaymentTransaction(
  ...
  const [suiCoins, assetCoins, { systemState }, { chainIdentifier }] =
    await Promise.all([
      discoverCoins(client, sender, SUI_TYPE_ARG),
      paysInSui
        ? Promise.resolve<OwnedCoin[]>([])
  ...
  const source =
    payment === null ? tx.gas : tx.objectRef(coinRef(payment.primary));
  ...
  if (available < reserve + minimumGas)
    throw new InsufficientBalanceError(
      SUI_TYPE_ARG,
      reserve + minimumGas,
      available
    );
  const gas = selectCoins(
    suiCoins,
    reserve + min(maxGasBudget, available - reserve),
    SUI_TYPE_ARG
  );
  tx.setGasPayment([gas.primary, ...gas.merge].map(coinRef));
  tx.setGasBudget(min(gas.total - reserve, PROTOCOL_MAX_TX_GAS));
  const simulated = await simulate(client, await tx.build());

  const gasBudget = computeGasBudget(
    simulated.effects.gasUsed,
    gasPrice,
    headroom
  );
  ...
  if (gasBudget > gas.total - reserve)
    throw new InsufficientBalanceError(
      SUI_TYPE_ARG,
      reserve + gasBudget,
      gas.total
  ...
  if (credited === null || credited < amount) {
    throw new PaymentBuildError(
      "self_check_failed",
      `simulation credits ${payTo} with ${
        credited ?? 0n
      } of ${asset}, expected at least ${amount}`
    );
  }
