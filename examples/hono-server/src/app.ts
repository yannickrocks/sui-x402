 ...
 */
import { x402, type Seller, type SellerOptions } from "@sui-x402/hono";
import { Hono } from "hono";

/** Synthetic: the example sells a fixed number, not market data. */
const QUOTE = "1.8400";

export function createApp(options: SellerOptions | Seller): Hono {
  const app = new Hono();

  app.get("/health", (c) => c.json({ ok: true }));

  // Registered before the route it guards: Hono runs handlers in the order they
  // were added, so a paywall added afterwards would never see the request.
  app.use("/paid/*", x402(options));

  app.get("/paid/quote", (c) =>
    c.json({ symbol: "SUI", quote: QUOTE, paidAt: new Date().toISOString() }),
  );
