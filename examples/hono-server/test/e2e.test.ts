...
import { ENV_PAYER_SECRET_KEY, SuiX402Payer, ed25519SignerFromEnv } from "@sui-x402/payer-sui";
...
const missing = ["E2E", ENV_PAYER_SECRET_KEY, "PAY_TO"].filter((name) => !process.env[name]);
if (missing.length > 0) {
  console.log(`e2e skipped: ${missing.join(" and ")} not set — see docs/BLOCKED.md #1`);
}

describe.skipIf(missing.length > 0)("hono example: 402 → pay → 200", () => {
  ...
  beforeAll(async () => {
    ...
    const info = await new Promise<AddressInfo>((resolve) => {
      server = serve(
        { fetch: app.fetch, port: 0, hostname: "127.0.0.1", serverOptions: { maxHeaderSize: MAX_HEADER_BYTES } },
        resolve,
      );
    });
  ...
  it(
    ...
    async () => {
      ...
      const payer = new SuiX402Payer({
        client: new SuiGrpcClient({ network: "testnet", baseUrl: env("SUI_GRPC_URL", DEFAULT_GRPC_URL) }),
      ...
      const { response, receipt } = await payer.fetchWithReceipt(`${base}/paid/quote`);
      expect(response.status).toBe(200);
      const body: unknown = await response.json();
      expect(body).toMatchObject({ symbol: "SUI", quote: expect.any(String), paidAt: expect.any(String) });

      if (receipt === null) throw new Error("200 answer carried no PAYMENT-RESPONSE header");
      expect(receipt.settle.success).toBe(true);
      expect(receipt.digest).not.toBe("");
      console.log(`e2e settled: https://testnet.suivision.xyz/txblock/${receipt.digest}`);
