...
A runnable x402 seller on [Express](https://expressjs.com): one free route and
one paid route, guarded by [`@sui-x402/express`](../../packages/middleware-express). Callers
...
```sh
cp examples/express-server/.env.example examples/express-server/.env
# set PAY_TO to an address you control (not the payer's own address)
set -a; source examples/express-server/.env; set +a
pnpm --filter example-express-server dev
```
...
```sh
set -a; source examples/express-server/.env; set +a
E2E=1 pnpm --filter example-express-server test
```
